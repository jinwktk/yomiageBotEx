...
Botを再起動する前に以下を必ず実行:
1. **全Pythonプロセス確認**: `tasklist | findstr python`
2. **重複プロセス終了**: `taskkill //f //im python.exe`
3. **単一プロセス起動**: 1つのBotプロセスのみ実行
...
**ユーザーからの指摘（2回目）**: 複数プロセス問題の再発防止のため、必ずこのルールに従うこと
...
### 2024-08-30 プロジェクト最終整理完了（第23回）
- **Serenaによる包括的プロジェクト整理**: 不要ファイル・フォルダ・コードの最終整理完了
- **古いログファイル削除**: yomiage_20250829_*.log.gz（3ファイル）を削除し、54KB削減
- **コード品質改善**: cogs/dictionary.pyのimport random不足を修正
- **プロジェクト構成最適化**:
  - 実効プロジェクトサイズ: 312KB（非常にコンパクト）
  - TTSキャッシュ: 428KB（必要分のみ保持）
  - 現在ログ: 93KB（開発用に保持）
- **CLAUDE.md更新**: 
  - Serena使用指針の追加
  - 作業メモ記録ルールの明確化
  ...
  - プロジェクト最終状態の文書化
- **プロジェクト最終状態確認**:
  - ✅ Discord Bot完全動作（7 Cogs、10スラッシュコマンド）
  - ✅ TTS読み上げ機能完全動作（Style-Bert-VITS2統合）
  - ✅ 自動参加・退出機能動作
  - ✅ 録音・リプレイ機能動作
  - ✅ 音声リレー機能動作（自動開始対応）
  - ✅ 辞書・ユーザー設定機能動作
...
### 2024-08-30 音声リレー機能のトラブルシューティング
- **問題**: ユーザーがチャンネルID `283439291227439106` に接続したが、`813783749153259606` への音声リレーが動作しない
- **原因**: 
  - `Source channel 283439291227439106 not found or not a voice channel`エラー
  - 指定されたソースチャンネルIDが正しくない可能性
- **対処実施**:
  - config.yamlのauto_relay_pairs設定を更新
  - `source_channel_id: 0` → `283439291227439106` に変更
  - ボット再起動で設定反映
- **現在の状況**: チャンネルIDの確認が必要
...
### 2025-08-30 TTS読み上げ機能完全修復とプロジェクト整理（最終完成）
- **Discord接続問題の根本解決**: `list index out of range`暗号化モード選択エラーを完全解決
- **解決手法**: キャッシュクリア + 依存関係再インストールによる接続状態リセット
- **自動再接続機能実装**: メッセージ送信時のボイスチャンネル自動再接続機能を追加
- **実装内容**:
  - `MessageReaderCog._attempt_auto_reconnect()`メソッド追加
  - 既存接続のクリーンアップと直接接続による安全な再接続
  - ユーザーがいるチャンネルの自動検出
  - 詳細なデバッグログとエラーハンドリング
- **TTS機能完全動作確認**:
  - ✅ メッセージ受信・処理: 正常
  - ✅ ボイスチャンネル接続: 自動参加成功
  - ✅ TTS API連携: 192.168.0.99:5000で正常動作
  - ✅ 辞書機能: 文字置換（TTS → ティーティーエス）動作
  - ✅ 音声生成・再生: FFmpeg処理完了
- **プロジェクト大規模整理実施**:
  - **削除対象**: Pythonキャッシュ、重複ログファイル、未使用スクリプト
  - **コード整理**: 未使用import削除、冗長コメント削除、デッドコード除去
  - **整理効果**: コード品質向上、起動時間短縮、保守性向上
- **CLAUDE.md大幅更新**:
  - **デフォルトツール指定**: Serenaをデフォルト使用に設定
  - **作業メモルール**: 全作業内容をCLAUDE.mdに必ず記録するルール追加
  - **プロジェクト指針**: 効率的な開発・保守のためのガイドライン整備

### 最終動作確認済み機能（2025-08-30現在）
- ✅ **自動参加・退出**: ユーザー参加時の自動VC参加、退出時の自動VC離脱
- ✅ **TTS読み上げ**: チャットメッセージの音声読み上げ（Style-Bert-VITS2統合）
- ✅ **録音・再生**: リアルタイム音声録音とリプレイ機能
- ✅ **辞書機能**: 単語置換による読み上げカスタマイズ
- ✅ **ユーザー設定**: 個人別TTS設定管理
- ✅ **音声横流し**: サーバー間音声リレー機能
- ✅ **自動再接続**: 接続切断時の自動復旧機能
...
- **ボット実行時は必ず1プロセスのみ**: 複数のPythonプロセスでbot.pyが動作することは禁止
- **起動前チェック**: bot.py実行前に既存プロセスの確認を行う
- **プロセス停止**: 既存のbot.pyプロセスが動作中の場合は停止してから新しいプロセスを開始
- **確認コマンド**: `tasklist | findstr python` (Windows) または `ps aux | grep python` (Linux/macOS)
- **理由**: 複数プロセスが動作するとDiscord接続の競合、音声リレーの重複、ログファイルの競合等が発生
